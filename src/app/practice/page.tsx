...
export default function PracticePage(){
  const {data,error}=useResource("practice",loadPractice);const [selected,setSelected]=useState(()=>{try{return localStorage.getItem("codeduel.practice.selected")??"";}catch{return "";}});
  ...
  return <AppShell><h1 className="mb-5 text-3xl font-semibold">Practice</h1><ErrorNotice message={error}/>
    ...
    {data&&<label className="mb-6 block">Choose a problem<select className="ml-3 max-w-full rounded border bg-white p-3" value={problem?.id??""} onChange={e=>{setSelected(e.target.value);try{localStorage.setItem("codeduel.practice.selected",e.target.value);}catch{/* Device preference is optional. */}}}>{data.problems.map(p=><option key={p.id} value={p.id}>{p.title} · {p.difficulty}</option>)}</select></label>}
...
function PracticeEditor({problem,userId}:{problem:RoundProblem;userId:string|null}) {
  ...
  async function run(){if(!userId||busy)return;setBusy(true);setError(null);try{const next=await api.runPractice(problem.slug,code);setResult(next);try{localStorage.setItem(storageKey+".result",JSON.stringify(next));}catch{/* Device history is optional. */}}catch(e){setError(e instanceof Error?e.message:"Run failed.");}finally{setBusy(false);}}
  return <div className="grid gap-6 lg:grid-cols-2"><ProblemPanel problem={problem}/><section className="rounded-xl bg-[#171717] p-6 text-white">
    ...
    <ErrorNotice message={error}/><p role="status" className="my-4">{result?`Last run: ${verdictLabel(result.verdict)} · ${result.testsPassed}/${result.testsTotal} public tests`:"Run your solution against public examples. Results do not affect rank."}</p>
