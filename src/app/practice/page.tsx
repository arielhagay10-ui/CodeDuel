...
function PracticeEditor({problem,userId,onSelectProblem,view,onBrowseProblems}:{problem:RoundProblem;userId:string|null;onSelectProblem:(id:string)=>void;view:"practice"|"stats";onBrowseProblems:()=>void}) {
  ...
  async function start(timed:boolean,abandonSessionId?:string){
    ...
    try{await api.startPracticeSession(problem.id,timed,abandonSessionId);setResumeActive(true);setResult(null);try{localStorage.removeItem(storageKey+".result");}catch{/* Optional history. */}if(ended){setCode(problem.starterCode);try{localStorage.setItem(storageKey,problem.starterCode);}catch{/* Optional draft storage. */}}setConfirmAbandon(false);refreshSession();}
  ...
  if(session?.timed&&!ended&&!resumeActive)return <section className="rounded-xl border border-[#c73d25]/30 bg-[#fbebe5] p-6"><h2 className="text-xl font-semibold">Timed attempt in progress · {problem.title}</h2><p className="my-4 text-sm">The clock has kept running. Resume this attempt, or end it and choose a practice mode.</p><ErrorNotice message={error??sessionError}/><div className="flex flex-wrap items-center gap-3"><button aria-label="Resume current timed attempt" className="inline-flex h-12 w-44 items-center justify-start gap-5 rounded-lg bg-[#ed5b39] px-5 text-sm font-bold text-white shadow-md ring-2 ring-[#ed5b39]/25 hover:bg-[#c73d25] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-2 focus-visible:outline-[#161616] disabled:opacity-40" disabled={busy} onClick={()=>setResumeActive(true)}><span className="translate-x-1" aria-hidden="true">▶</span><span>Resume timed</span></button><button className={`${buttonClass} inline-flex h-12 w-44 items-center justify-center`} disabled={busy} onClick={()=>void start(false,session.id)}>Continue untimed</button><button className={`${buttonClass} inline-flex h-12 w-44 items-center justify-center`} disabled={busy} onClick={()=>void start(true,session.id)}>Restart timed</button></div></section>;
